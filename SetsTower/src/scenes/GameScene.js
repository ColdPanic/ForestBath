...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(
      GRID_X + 2,
      GRID_Y + 2,
      'S', { fontSize: '10px', color: '#44ff88', fontStyle: 'bold' }
    ).setOrigin(0, 0).setDepth(6)

    this.add.text(
      GRID_X + 8 * (CARD_W + GAP) + CARD_W - 2,
      GRID_Y + 8 * (CARD_H + GAP) + CARD_H - 2,
      'F', { fontSize: '10px', color: '#ffcc00', fontStyle: 'bold' }
    ...
    // ── UI (depth 10) ─────────────────────────────────────────────────────────
    this._scoreTxt = this.add.text(W / 2, 16, 'Sets: 0', {
      fontSize: '22px', color: '#ffffff', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(10)

    this._statusTxt = this.add.text(W / 2, 46, 'Find a Set — start near the cyan tiles', {
